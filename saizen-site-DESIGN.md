...
### 11d. Top bar wordmark and theme toggle, corrected against the real app source

The top bar wordmark previously used a site-only invention — wide-tracked uppercase mono ("SAIZEN"), no basis in the app. Pulled the real header source instead (`apps/web/src/app/AppShell.tsx` + the `.text-brand` utility in `apps/web/src/app/globals.css` in `Surajsm60720/Saizen`) and matched it exactly: Quando, weight 700, `line-height:1`, `letter-spacing:-0.01em`, sentence case ("Saizen," not "SAIZEN"), sized 18px→20px (the app's breakpoint jump reimplemented here as a fluid `clamp()` to match this site's own convention, not the app's discrete jump). The app's header has no version label and no theme control — both stay site-only additions, consistent with the app having no light/dark theming at all (§10's known trap).

The theme toggle dropped its bordered button chrome and text label entirely — it's now just the diagonal blade mark, icon-only, in a 44×44 hit area with no visible box. The mark is the same motif the click triggers (the blade-wipe sweep), so the control previews its own action rather than needing a "LIGHT/DARK" label to explain it.

The blade transition's cleanup was also hardened: it previously used two `setTimeout`s — one for the swap (unavoidable; CSS has no "sweep is 40% done" event to hook), and a second one guessing the animation's total duration to know when to reset. That guess had already drifted out of sync once when the animation duration was tuned (620ms → 900ms) without updating both timers to match. Cleanup now listens for the CSS animation's own `animationend` event instead, so it can't drift regardless of what duration `globals.css` ends up using — with a 2s safety-net timeout in case the event is ever missed (tab backgrounded mid-sweep) so the toggle can never get permanently stuck.
